...
% --- CHUNK_METADATA_END ---
\begin{theorem}
Let $\mathcal{M}_1(\mathbb{R})$ denote the set of probability measures on $(\mathbb{R}, \mathcal{B}(\mathbb{R}))$, and let $\mathfrak{R}$ denote the set of functions satisfying the three properties of a distribution function. The mapping $\mu \mapsto F_\mu$, where $F_\mu(x) = \mu((-\infty, x])$, is a bijection from $\mathcal{M}_1(\mathbb{R})$ to $\mathfrak{R}$.
