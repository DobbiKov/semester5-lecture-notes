...
% needs_review: True
% --- CHUNK_METADATA_END ---
\begin{example}[Théorème de convergence dominée][Dominated Convergence Theorem]
...
% needs_review: True
% --- CHUNK_METADATA_END ---
\begin{example}[La suite "machine à écrire"][La suite "machine à écrire"]
...
% --- CHUNK_METADATA_END ---
\begin{definition}
An infinite sequence of σ-fields $(\mathcal{F}_n)_{n \in \mathbb{N}}$ is said to be independent (resp. of events, of random variables) if every finite family extracted from the sequence is independent.
\end{definition}% --- CHUNK_METADATA_START ---
...
Let $(X_n)_{n \in \mathbb{N}}$ be a sequence of independent random variables. Let $(I_k)_{k \in \mathbb{N}}$ be a partition of $\mathbb{N}$ (pairwise disjoint sets whose union is $\mathbb{N}$). Then the σ-algebras $\mathcal{J}_k = \sigma(X_n, n \in I_k)$ for $k \in \mathbb{N}$ are independent.
